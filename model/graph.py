...
class BasicLayer(nn.Module):
    def __init__(self, adjacency_matrix, use_learned_adj=True, padding=0):
        super().__init__()
        ...
        predefined_adj = torch.tensor(adjacency_matrix, dtype=torch.float32, device=args.device)
    ...
    def forward(self, feat):
        ...
        if self.padding > 0:
            pad = torch.zeros(size=(batchsize, self.padding, args.num_nodes, args.hidden_dim),
                              device=args.device)
        ...
        for i in range(2, feat.size(1)):
            ...
            if self.use_learned_adj:
                ...
                feature_with_learned_adj = self.ffn(feature_with_learned_adj[:, -args.num_nodes:, :])
                ...
                feature_sum = feature_sum + self.ffn(feature_with_predefined_adj[:, -args.num_nodes:, :])
...
class GraphBranchBackbone(nn.Module):
    def __init__(self, num_layers):
        super(GraphBranchBackbone, self).__init__()
        self.layers = nn.Sequential(*[BasicLayer(args.adj, use_learned_adj=False, padding=2) for j in range(num_layers)])

    def forward(self, feature):
        feature = self.layers(feature)  # B x T x N x D
        return feature


class GraphBranch(BasicModel):
    def __init__(self, *params, **options):
        super().__init__(*params, **options)
        self.backbone = GraphBranchBackbone(num_layers=args.stgcn_num_layers)
